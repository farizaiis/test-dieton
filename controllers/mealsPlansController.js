...
module.exports = {
    postMealsPlans: async (req, res) => {
        ...
        try {
            ...
            if (!dataCreate) {
                return res.status(400).json({
                    status: 'fail',
                    message: 'Fail Create Meals Plan',
                });
            }
            ...
            if (
                ...
                return res
                    ...
                    .json({
                        status: 'failed',
                        message:
                            'Breakfast, Lunch, or Dinner only for meals time',
                    });
            }
    ...
    updateStatus: async (req, res) => {
        try {
            ...
            const updateStatus = await mealsPlans.update(
                {
                    status: 1,
                },
                {
            ...
            if (!updateStatus) {
                return res.status(400).json({
                    status: 'failed',
                    message: 'Fail to update status Meals Plan',
                });
            }
            ...
            await calorieTrackers.update(
                {
                    calConsumed:
                        cekCalorieTracker.dataValues.calConsumed +
                        cekMealsPlans.dataValues.totalCalAmount,
                    remainCalSize:
                        cekCalorieTracker.dataValues.remainCalSize -
                        cekMealsPlans.dataValues.totalCalAmount,
                },
                {
                    where: {
